...
describe('CLI Integration', function () {
    ...
    it('resolves relative paths correctly', function () {
        ...
        $ref = new ReflectionMethod($command, 'resolvePath');
        $ref->setAccessible(true);

        $cwd = getcwd();

        expect($ref->invoke($command, 'file.env'))->toBe($cwd.'/file.env');
        expect($ref->invoke($command, './other.env'))->toBe($cwd.'/other.env');
        expect($ref->invoke($command, '../up.env'))->toBe($cwd.'/../up.env');
    });
